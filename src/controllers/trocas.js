...
controller.create = async function(req, res) {
  try {
    ...
    await prisma.trocas.create({data: req.body})
...
controller.retrieveAll = async function(req, res) {
  try {
      ...
      if(req.query.produtoServico)   include.produtoServico = {include: {trocas: true, avaliacoes: true}}

    // Manda buscar os dados no servidor
    // Traz ordenado por nome, depois por nivel
    const result = await prisma.trocas.findMany({
      include,
      orderBy: [
...
controller.retrieveOne = async function(req, res) {
  try {
    const result = await prisma.trocas.findUnique({
...
controller.update = async function(req, res) {
  try {
    const result = await prisma.trocas.update({
      where: { id: req.params.id },
      data: req.body
    })
...
controller.delete = async function(req, res) {
  try {
    const result = await prisma.trocas.delete({
      where: { id: req.params.id }
    })
